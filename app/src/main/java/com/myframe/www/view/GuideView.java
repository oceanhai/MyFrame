...
public class GuideView extends RelativeLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private boolean first = true;//是否是首次设置 提示描述/图片
    /**
    ...
    public void showOnce() {
        if (targetView != null && sharedPreferencesKey != null) {
            mContent.getSharedPreferences(TAG, Context.MODE_PRIVATE).edit().putBoolean(sharedPreferencesKey, true).commit();
        }
    }
    ...
    public boolean hasShown() {
        if (targetView == null || sharedPreferencesKey == null)
            return true;
        return mContent.getSharedPreferences(TAG, Context.MODE_PRIVATE).getBoolean(sharedPreferencesKey, false);
    }
    ...
    public void show() {
        ...
        if (targetView != null) {
            targetView.getViewTreeObserver().addOnGlobalLayoutListener(this);//注册监听视图树的观察者
        }

        this.setBackgroundResource(R.color.transparent);

        ((FrameLayout) ((Activity) mContent).getWindow().getDecorView()).addView(this);//蒙层view 添加到根view
        first = false;
    }
    ...
    boolean needDraw = true;//TODO 没有实际用处
    ...
    /**
     * buid 时候，注册 guideview点击监听（调用GuideView Callback ※其实直接在外部设置"点击监听"不就好了，为啥还套一层？）
     */
    ...
    public static class Builder {
        ...
        public Builder setSharedPreferencesKey(String key) {
            guiderView.setSharedPreferencesKey(key);
            return instance;
        }
